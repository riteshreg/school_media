    ...
    <ul className="space-y-2 my-5 " key={message.id}>
      ...
      {message.author == loginUserData?.id && (
        <li className="flex justify-end">
          ...
          <div> {message.images?.length > 0 && (<div className={`${message.images.length==1 && "flex  justify-end"} ${message.images.length>1 && "grid gap-2 grid-cols-2"} `}>{ message.images?.map((imag)=>
              <Image
              key={imag}
                alt="image"
                height={180}
